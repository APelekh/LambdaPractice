...
namespace LambdaPractice
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(products.OrderBy(x => x.Length).ToList()[2]);
            ...
            List<string> ballProductNew = ballProducts.OrderByDescending(x => x.Length).Skip(1).Take(1).ToList();

            foreach (string item in ballProductNew)
            {
                Console.WriteLine(item);
            }
            ...
            foreach (string item in products.OrderByDescending(x => x.Length).ToList())
            {
                Console.WriteLine(item);
            }
